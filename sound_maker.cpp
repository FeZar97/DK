                         ...
                         SDR_params *new_sdr_params)
{
    ...
    raw_file = new QFile("after filtration.pcm");
    raw_file->open(QIODevice::WriteOnly);

    out_file = new QFile("after demodulation.pcm");
...
void sound_maker::get_sound_step(Ipp32fc *rb_cell, unsigned int cell_size)
{
    if(sdr_params->sample_rate != 1440000)
        ...
        raw_file->write(reinterpret_cast<char*>(rb_cell), cell_size * sizeof(Ipp32f));
        ...
        /*
         ...
         sample_rate = 48kHz = 4800 сэмплов 16s per 0,1 sec = 9600 byte per 0,1 sec
        ...
        audio_device->write(reinterpret_cast<char*>(out_buf), BUF_SIZE * sizeof(Ipp16s));
        out_file->write(reinterpret_cast<char*>(out_buf), BUF_SIZE * sizeof(Ipp16s));
    }
